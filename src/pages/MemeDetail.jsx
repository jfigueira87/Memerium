...
const MemeDetail = () => {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col md:flex-row justify-center items-center bg-secundary py-8 space-y-10 md:space-y-0 md:space-x-10">
      {/* Imagen del meme con marco */}
      ...
      <div className="w-full md:w-1/2 text-white font-mainFont space-y-4 text-center md:text-left p-5">
        <h2 className="text-xl md:text-2xl font-semibold">Título: {memeData.name}</h2>
        <p className="text-lg break-words">
          URL: <span className="break-all">{memeData.url}</span>
        </p>
        ...
        <div className="flex flex-col md:flex-row justify-center md:justify-start items-center space-y-4 md:space-y-0 md:space-x-4 mt-4">
          <button
            ...
            className="md:w-auto bg-colorButton text-primary font-semibold py-2 px-10 rounded-md shadow-md hover:bg-gray-400"
          >
            Eliminar meme
          </button>
          <Link
            to={`/editmeme/${id}`}
            className=" md:w-auto bg-colorButton text-primary font-semibold py-2 px-10 rounded-md shadow-md hover:bg-gray-400 text-center"
          >
            Editar información
          </Link>
        </div>
